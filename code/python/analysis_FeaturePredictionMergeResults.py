...
def main():
    ...
    cor_list = [corrcoef(t, p, var='col')
                for t, p in zip(res_pt.loc['true_feature_averaged'],
                                res_pt.loc['predicted_feature_averaged'])]

    print(numpy.array(cor_list).shape)

    res_pt.loc['profile_correlation_image'] = cor_list
    res_pt.loc['mean_profile_correlation_image'] = res_pt.loc[:, 'profile_correlation_image'].apply(np.nanmean)

    # Profile correlation (category, seen)
    res_pt.loc['profile_correlation_cat_percept'] = [corrcoef(t, p, var='col')
                                                     for t, p in zip(res_pt['category_feature_averaged'],
                                                                     res_pt['predicted_feature_averaged'])]
    res_pt.loc['mean_profile_correlation_cat_percept'] = res_pt.loc[:, 'profile_correlation_cat_percept'].apply(
        np.nanmean)

    # Profile correlation (category, imagined)
    res_im.loc['profile_correlation_cat_imagery'] = [corrcoef(t, p, var='col')
                                                     for t, p in zip(res_im['category_feature_averaged'],
                                                                     res_im['predicted_feature_averaged'])]
    res_im.loc['mean_profile_correlation_cat_imagery'] = res_im.loc[:, 'profile_correlation_cat_imagery'].apply(
        np.nanmean)
